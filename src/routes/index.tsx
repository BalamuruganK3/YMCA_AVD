        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between border-b border-border/60 pb-2">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-semibold uppercase text-foreground">{area.label}</div>
                <div className="text-xs text-muted-foreground">{14} rooms</div>
              </div>
            </div>
              ...
              {Array.from({ length: totalPages }, (_, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all cursor-pointer ${
                    pageIndex === i ? "w-6 bg-primary" : "w-2 bg-muted hover:bg-muted-foreground/50"
                  }`}
...
function NewRoomDialog() {
  ...
  const { data: customAreas = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("rooms").select("area").neq("area", "admin_room");
      if (error) throw error;
      const slugSet = new Set<string>();
      for (const row of data ?? []) {
        const slug = String(row.area)
          .trim()
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-")
          .replace(/^-+|-+$/g, "");
  ...
  const updateWork = (idx: number, patch: Partial<(typeof works)[number]>) => {
  ...
  const createRoom = useMutation({
    mutationFn: async () => {
      ...
      const validWorks = works.filter(
        (w) => w.heading.trim() && w.subheading.trim() && w.work.trim(),
      );
        ...
        <div className="space-y-4 py-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  {customAreas.length > 0 && (
                    <SelectItem
                      value={`__custom_${customAreas.join("__")}`}
                      disabled
          ...
          {customAreaPreset && (
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                Adding another room to the{" "}
                <span className="font-medium">{titleCase(customAreaPreset)}</span> area you created
                earlier.
              </p>
            </div>
          )}
...
function PrintReportDialog({ trigger }: { trigger: (o: { open: () => void }) => ReactNode }) {
  const queryClient = useQueryClient();
  const [open, setOpen] = useState(false);
      ...
      if (itemIds.length > 0) {
        ...
        itemsMap = new Map(
          (items ?? []).map((it) => {
            ...
            return [it.id, { title: it.title, roomName: rm?.name ?? "", area: rm?.area ?? "" }];
          }),
        );
      }
...
function DashboardPage() {
  ...
  const { data: rawRooms = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (selectedDate) {
        ...
        const itemIds = (data ?? []).flatMap((room) =>
          (room.work_items ?? []).map((item) => item.id),
        );
  ...
  const knownSlugs: string[] = AREAS.map((a) => a.slug);
              ...
              <div className="p-4">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className="text-xs text-muted-foreground max-w-[16rem]">
                    Add a new room (choose a type like Staff Room, Admin Room or Smart Class) and a
                    number. It will appear as a new tab above with its default works.
                  </div>
                </div>
              </div>
            </section>
