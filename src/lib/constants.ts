...
export const DEFAULT_AREA_ROOMS: Record<AreaSlug, string[]> = {
  ...
  lab: ["CS lab", "Biology lab", "Chemistry lab", "Physics lab", "stem lab"],
...
export function getRoomProgress(
  room: {
    name: string;
    work_items?: { status: string; remarks?: string | null; title?: string }[] | null;
  },
  areaSlug: AreaSlug,
): number {
...
export function getAreaOverallProgress(
  areaSlug: AreaSlug,
  areaRooms: {
    name: string;
    work_items?: { status: string; remarks?: string | null; title?: string }[] | null;
...
export function getEffectiveAreaRooms<T extends { id: string; area: string; name: string }>(
  ...
  // Default rooms first, with virtual placeholders for any that don't exist yet.
  for (const name of expectedNames) {
    ...
    if (found) {
      ...
      result.push({
        id: `virtual-${slug}-${result.length + 1}`,
        area: slug,
        name,
        work_items: [],
      } as unknown as T);
  ...
  // Append any extra real rooms created by staff (beyond the default set)
  // so newly added rooms appear on the dashboard and room selector.
